...
export const loginAuthentication = async (loginDto: LoginDto) => {
    ...
    // Lưu ý: Không cần truyền full URL (https://...), chỉ cần endpoint
    const response: any = await axiosClient.post('/authentication/token', loginDto);

    // Logic lưu token: Kiểm tra cấu trúc response trả về từ Server của bạn
    // Giả sử server trả về: { code: 1000, result: { token: "..." } }
    if (response && response.result && response.result.token) {
        localStorage.setItem('accessToken', response.result.token); // Lưu token
    }
