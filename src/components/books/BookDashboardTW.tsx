...
import RightPanel from "../layout/RightPanelTW";
import BookCard from "./BookCardTW";
import Sidebar from "../layout/SidebarTW";
...
/**
 * Trang dashboard dành cho người dùng mượn sách.
 * - Lấy dữ liệu tổng quan (best sellers, recently read, wishlist, genres).
 * - Xử lý đăng nhập nhẹ tại chỗ (modal) để tránh bị out-of-flow.
 * - Cho phép thêm sách vào giỏ mượn + mượn nhanh 1 cuốn qua BorrowModal.
 *
 * Cố ý tách nhỏ các khối render (modals, hero, filters) thành các hàm con
 * để khi thầy hỏi có thể chỉ vào từng phần: "data -> filter -> list -> actions".
 */
const BookDashboard: React.FC = () => {
    ...
    // Lọc dữ liệu hiển thị: chỉ dựa trên text + chủ đề
    ...
    const handleLoginSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Lỗi đã được context quản lý sẵn
        }
    };
    ...
    const handleConfirmBorrow = async (days: number) => {
        const storedUser = getStoredUser() || JSON.parse(localStorage.getItem("currentUser") || "null");
        if (!storedUser || !borrowBook) {
        ...
        try {
            ...
            // Fallback local storage: vẫn cho mượn để user không bị kẹt flow
    ...
    /**
     * Các block render riêng: giúp trả lời "phần này làm gì" nhanh hơn.
     */
    const renderHero = () => (
    ...
    return (
        <div className="relative flex min-h-screen bg-gradient-to-b from-[#ffe6d0] via-[#fff5ea] to-white text-gray-900">
            ...
            {showFavorites && renderFavoritesModal()}
