...
public class ReadingHistoryController {
    ...
    @GetMapping("/{bookId}")
    public APIResponse<List<ReadingHistory>> getReadingHistoryByBook(@PathVariable Long bookId) {
        APIResponse<List<ReadingHistory>> response = new APIResponse<>();
        response.setResult(historyService.getReadingHistoryByBook(bookId));
        return response;
    }
}