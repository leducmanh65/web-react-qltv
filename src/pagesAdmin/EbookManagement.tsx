...
import { Loader2, Eye, UploadCloud, Search, Trash2, Edit } from "lucide-react"; // Thêm Edit icon
import CreateEbookForm from "../components/forms/create/CreateEbookForm";
import EditEbookForm from "../components/forms/edit/EditEbookForm"; // Import Form Sửa
...
export default function EbookManagement() {
    ...
    const openEdit = (pageId: number, bookId: number) => {
    ...
    return (
        <div>
                                    ...
                                    <tr key={g.bookId}>
                                        ...
                                        <td>
                                            <div className="action-group">
                                                <button className="icon-btn" onClick={() => setViewerOpenFor({ id: g.bookId, title: g.title })} title="Xem tất cả trang">
                                                    <Eye size={18} />
                                                </button>
                                                
                                                <button className="icon-btn" onClick={() => openUpload(g.bookId)} title="Thêm trang">
                                                    <UploadCloud size={18} />
                                                </button>

                                                {/* Nút Sửa (Sửa trang đầu tiên hoặc trang đại diện) */}
                                                <button className="icon-btn" onClick={() => openEdit(g.pageIds[0], g.bookId)} title="Sửa trang đầu">
                                                    <Edit size={18} />
                                                </button>

                                                <button className="icon-btn danger" onClick={() => handleDeletePage(g.pageIds[0])} title="Xóa trang đầu">
                                                    <Trash2 size={18} />
                                                </button>
                                                
                                                <button className="btn btn-sm btn-danger" onClick={() => handleDeleteAllPages(g.pageIds, g.title)} title="Xóa tất cả">
                                                    Xoá tất cả
                                                </button>
                                            </div>
                                        </td>
            ...
            <EditEbookForm
                ...
                onClose={() => { setIsEditModalOpen(false); setEditPageId(null); }}
